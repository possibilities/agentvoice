...
describe("native restart handoff", () => {
  ...
  test("native task input preserves default prompts and refuses stale/shutdown targets", async () => {
    ...
    try {
      ...
      expect((await h.runtime.submitHandoff({ ...input, threadId: "foreign" })).status).toBe(
        "failed",
      );
