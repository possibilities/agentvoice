...
for (const version of [
  ...
  test(`voice viewer handles controller event protocol ${version}`, async () => {
      ...
      socket.on("data", (chunk) => {
        ...
        for (const line of lines) {
          ...
          socket.end(
            [
              { ...response, ok: true },
              event("voice.item.started", segment("user", "ignore")),
    ...
    try {
      ...
      for (const request of requests)
        expect(request.params).toEqual({ events: ["voice.item.completed"] });
      if (version !== EVENT_PROTOCOL_VERSION) {
        ...
        expect(stderr).toContain("Listening for completed voice messages");
      }
    } finally {
