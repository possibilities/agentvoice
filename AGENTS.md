...
- `bun run android:configure` — discover running Studio devices and choose one
  ...
  Save captures every visual choice in profile 24; protocol 31 carries current,
...
## Independent AgentHUD

AgentHUD now owns its Work store, CLI/MCP, web UI, installer and skill in the
separate `~/code/agenthud` repository. AgentVoice exports only bounded read-only
metadata through `agentvoice threads --json` (`src/threads/export.ts`). Preserve
that versioned contract and exact native identity without credentials or socket
paths. Missing/incomplete native observation never settles durable Work.
See [the transfer decision](docs/adr/0056-independent-agenthud.md). AgentStart
owns the HUD LaunchAgent; changing HUD never requires restarting this call server.
...
new_session preflights, drains the old runtime, clears the marker, then creates and
saves a thread and reconnects voice. Old history remains.
...
work, attachment gateway, operation journal, endpoints, and workspace/thread
...
new_session also replaces the exact root. Explicit
...
- No AgentVoice runtime worker execution tools, registry, archival or result reports.
  ...
  Automated custom turn submission is limited to explicit MCP/API restart handoffs ([ADR 0016](docs/adr/0016-restart-handoff.md)). Explicit human Agent composer input uses the exact-thread attachment gateway; see [web input](docs/web-agent-input.md). Child lifecycle remains observation only; native Codex returns authoritative child results to the exact parent ([ADR 0103](docs/adr/0103-retire-synthetic-subagent-lifecycle-steering.md)). Restart handoffs require live media. Never retry ambiguous acceptance,
