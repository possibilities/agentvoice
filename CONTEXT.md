...
UI removal does not retire API controls. See `docs/api.md`.
...
the old runtime, clears the workspace marker, creates and saves a
...
**Native direct-child result return** — Codex sends a terminal child's authoritative
response to its exact native parent as queued `InterAgentCommunication`. AgentVoice
does not synthesize a wake-up, `turn/start`, steer, lifecycle summary, mailbox, or
result store from child observations. The result can join an active wait or the
