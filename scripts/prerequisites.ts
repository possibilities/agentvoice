...
export function whichFromEnvironment(command: string): string | null {
...
export function checkPrerequisites(): { codex: string; compiler: string } {
  ...
  const codex = whichFromEnvironment(process.env["CODEX_PATH"] ?? "codex");
  ...
  const compiler =
    whichFromEnvironment("zig") ?? whichFromEnvironment("clang") ?? whichFromEnvironment("cc");
