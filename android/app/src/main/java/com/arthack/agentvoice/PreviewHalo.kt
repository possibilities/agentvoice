...
internal data class PreviewHalo(
    ...
    init {
        ...
        require(thinkingWingspan in 1..10)
    ...
    fun json(): JSONObject = JSONObject().put("variant", variant).put("containedSizePercent", containedSizePercent)
        ...
        .put("thinkingWingspan", thinkingWingspan)
    ...
    fun tuning() = CompactHaloTuning(ringSpreadPercent, listeningPulsePercent, speakingMotionPercent, idleBreathingPercent, thinkingWingspan)
...
internal fun decodePreviewHalo(data: JSONObject, legacyWingspan: Boolean = false): PreviewHalo {
    require(data.fields() == setOf("variant", "containedSizePercent", "ringSpreadPercent", "listeningPulsePercent",
        "speakingMotionPercent", "idleBreathingPercent", "colors") + (if (legacyWingspan) emptySet<String>() else setOf("thinkingWingspan")))
    ...
    return PreviewHalo(data.getString("variant"), percent("containedSizePercent", 35, 120), percent("ringSpreadPercent"),
        ...
        color("speaking"), color("listening"), color("idle"), if (legacyWingspan) 2 else percent("thinkingWingspan", 1, 10))
}
...
internal fun decodePersonaHalo(json: String): PreviewHalo {
    ...
    return when (data.getInt("version")) {
        1, 2, 3, 4 -> PreviewHalo()
        in 5..23 -> decodePreviewHalo(data.getJSONObject("halo"), legacyWingspan = data.getInt("version") <= 22)
