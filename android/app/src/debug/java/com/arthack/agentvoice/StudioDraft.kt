...
internal fun PersonaPreviewState.designProfile(): String {
    ...
    return JSONObject(encodePersonaTuning(portrait.placement, portrait.design, portrait.halo, portrait.spirit,
        ...
        sharedAppearance, sounds, appearance(), layouts.getValue(previewPortraitReverse),
        layouts.getValue(previewLandscapeReverse))).put("savedAtEpochMs", 1).toString()
}
...
internal fun PersonaPreviewState.withDesignProfile(profile: String): PersonaPreviewState {
    ...
    return endHold().withLayoutMaps(layouts.asMap(), savedLayouts()).withAppearance(decodeDesignAppearanceProfile(profile)).copy(
...
internal class StudioDraft(private val file: File, private val production: String = StudioProduction.profile) {
    ...
    fun open(): String {
        ...
        if (previous != null) {
            ...
            require(data.getInt("version") in 1..3) { "Unsupported Studio draft" }
    ...
    fun write(profile: String) {
        ...
        validate(profile)
        savePersonaTuning(file, JSONObject().put("version", 3)
