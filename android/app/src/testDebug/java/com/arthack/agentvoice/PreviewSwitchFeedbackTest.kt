...
class PreviewSwitchFeedbackTest {
    ...
    @Test fun explicitGesturesHaveDirectionalMuteCuesAndOneRelatedPttPair() {
        ...
        feedback.toggle(true); feedback.toggle(false); feedback.down(); feedback.down(); feedback.release(); feedback.release()
        assertEquals(listOf(PreviewSwitchCue.ToggleOn, PreviewSwitchCue.ToggleOff, PreviewSwitchCue.Down, PreviewSwitchCue.Up), output.events.map { it.second })
    ...
    @Test fun offVolumeAndBackgroundNeverPlayOrRetainARelease() {
        ...
        for ((settings, active) in listOf(PreviewSounds() to true, PreviewSounds("rocker-13", 0) to true,
            ...
            feedback.update(settings, active); feedback.toggle(true); feedback.down(); feedback.release()
        }
        assertTrue(output.events.isEmpty())
    ...
    @Test fun notReadyNeverQueuesOrProducesAnOrphanRelease() {
        val output = Output(); val feedback = PreviewSwitchFeedback(output)
        feedback.update(PreviewSounds("rocker-13", 70), true)
        output.ready = false; feedback.down(); feedback.toggle(true)
