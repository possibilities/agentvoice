...
class PreviewProfileTwelveTest {
    ...
    private fun preview(state: PersonaPreviewState) = state.activeLayout().json()
        ...
        .put("theme", state.theme).put("mutedPresence", state.mutedPresence).put("mutedTuning", state.mutedTuning.json()).put("presenceScope", state.presenceScope).put("sounds", state.sounds.json()).put("showPushToTalk", state.showPushToTalk).put("icons", state.icons.json()).put("launcher", state.launcher).put("connectionStyle", state.connectionStyle).put("notificationStyle", state.notificationStyle)
    ...
    @Test fun presentationIsRestoredAndSavedAtRootWhileOldRequestsRemainFenced() = runBlocking {
        ...
        try {
            ...
            assertEquals(24, encoded.getInt("version"))
            ...
            assertTrue(reply.toString().toByteArray().size < 65536)
    ...
    @Test fun indicatorOptionsRoundTripAndProtocolFourteenAddsOnlyDefaultScope() = runBlocking {
        ...
        try {
            for (style in previewMutedPresences) for (scope in previewPresenceScopes) {
                ...
                assertEquals(31, current.json().getInt("protocol"))
            ...
            assertEquals(original.copy(presenceScope = "any-muted").withLegacyAxisClones(), restorePersonaPreview(legacy,
