...
class PreviewSoundsSessionTest {
    private fun preview(state: PersonaPreviewState) = state.activeLayout().json()
        ...
        .put("presenceScope", state.presenceScope).put("sounds", state.sounds.json()).put("showPushToTalk", state.showPushToTalk).put("icons", state.icons.json()).put("launcher", state.launcher).put("connectionStyle", state.connectionStyle).put("notificationStyle", state.notificationStyle)
    ...
    @Test fun sharedSelectionSurvivesRotationAndSavesOnceAtTheProfileRoot() = runBlocking {
        ...
        try {
            ...
            assertEquals(31, stateJson.getInt("protocol"))
    ...
    @Test fun oldProfilesAndSessionsDefaultOffWithoutRewritingTheirBytes() {
        ...
        try {
            ...
            val legacyCurrent = previous.copy(
                design = previous.design.copy(controlsWithoutPttDp = previous.design.controlsHeightDp),
                otherLayout = previous.otherLayout.copy(design = previous.otherLayout.design.copy(
                    controlsWithoutPttDp = previous.otherLayout.design.controlsHeightDp)))
            assertEquals(legacyCurrent.withLegacyAxisClones(), restore(oldSession, previous))
    ...
    @Test fun currentProfileAndRestoreRequireCompleteSoundContracts() {
        ...
        assertEquals(state.savedSounds, restored.savedSounds)
    }
