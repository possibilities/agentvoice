...
class ShippingProfileTest {
    private val appearanceFields = setOf("theme", "mutedPresence", "mutedTuning", "presenceScope", "showPushToTalk", "icons")
    ...
    private fun preview(state: PersonaPreviewState) = state.activeLayout().json()
        ...
        .put("icons", state.icons.json()).put("sounds", state.sounds.json())
    ...
    @Test fun profileEighteenUsesHistoricalAppearanceWithoutChangingSavedBytesOrLayouts() = runBlocking {
        ...
        try {
            ...
            assertEquals(19, JSONObject(migrated).getInt("version"))
    ...
    @Test fun profileNineteenRequiresCompleteValidAppearance() {
        ...
        val invalid = listOf(
            "theme" to "unknown", "mutedPresence" to "unknown", "presenceScope" to "unknown",
