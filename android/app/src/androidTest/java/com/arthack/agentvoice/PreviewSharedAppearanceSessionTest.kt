...
class PreviewSharedAppearanceSessionTest {
    private fun request(state: PersonaPreviewState) = state.activeLayout().json()
        ...
        .put("mutedTuning", state.mutedTuning.json()).put("presenceScope", state.presenceScope).put("sounds", state.sounds.json()).put("showPushToTalk", state.showPushToTalk).put("icons", state.icons.json()).put("launcher", state.launcher).put("connectionStyle", state.connectionStyle).put("notificationStyle", state.notificationStyle)
    ...
    @Test fun sharedAppearanceEditsEitherOrientationWhileOverridesAndGeometryStayLocal() = runBlocking {
        ...
        try {
            ...
            assertEquals(9, shared.otherLayout.halo.thinkingWingspan)
            ...
            assertTrue("Expanded profile receipt remains bounded", reply.toString().toByteArray().size < 65536)
