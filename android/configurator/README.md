...
the preview between displays, recreating its activity or backgrounding it ends
the preview session, so rerun the host command after such a change.
...
Reopening the phone preview or reconnecting a disconnected device requires
explicitly rerunning the host command.
...
A changed phone revision refuses a stale save. A failed host write is reported
...
debug preview. Its separate random token admits one session. Frames are limited
...
activity stop, host loss, invalid framing or liveness failure. It is absent from
...
and coordinates saves; `src/device.ts` owns the selected ADB connection;
