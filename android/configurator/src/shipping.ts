...
export function createShippingSnapshot(
  ...
  if (profile.version !== 18 && profile.version !== 19)
    throw Error("Promotion requires profile18 or profile19");
  ...
  if (choice.kind === "defaults") appearance = defaultVisualSettings();
    ...
    if (state["protocol"] !== 21 && state["protocol"] !== 22)
    ...
    appearance = visualSettingsOf(state);
  }
  return {
    version: 1,
    source: {
...
export function parseShippingSnapshot(text: string): ShippingSnapshot {
  ...
  const appearance = parseVisualSettings(value["appearance"]);
  if (digest(session["valuesSha256"]) !== sha256(canonicalJson(appearance)))
...
): Extract<Profile, { version: 19 }> {
  const source = parseProfile(snapshot.source.profile.text);
  if (source.version !== 18 && source.version !== 19)
    throw Error("Unsupported shipping profile source");
  return { ...source, version: 19, ...snapshot.appearance };
}
