/**
 * The mute doctrine. A channel has a persistent mute assignment and any number
 * of momentary unmute holds layered over it; the last hold released closes the
 * channel again. Push-to-talk is the microphone case. Every input source
 * releases only its own hold, so a lost key release cannot strand a live mic.
 */
...
export function audioControlKeyAction(key: ControlKey): AudioControlKeyAction | null {
  const target = key.name === "m" ? "mic" : key.name === "s" ? "speaker" : null;
  if (!target) return null;
  if (key.source === "kitty" && key.eventType === "release") return { target, action: "end" };
  if (key.source === "raw") {
...
export function spaceControlKeyAction(key: ControlKey): SpaceControlKeyAction | null {
...
export interface MuteState {
  /** The persistent mute assignment controlled by clicks. */
  muted: boolean;
...
export class MuteGate {
  ...
  state(): MuteState {
    return { muted: this.muted, holding: this.holding, effectiveMuted: this.effectiveMuted };
  }
