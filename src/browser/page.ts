...
<style>
  ...
  main { width: min(28rem, calc(100% - 2rem)); display: grid; gap: 1rem; }
  button { min-height: 4rem; font: inherit; }
...
<main>
  ...
  <button id="hold" type="button" disabled hidden>Hold to talk</button>
...
export const browserMediaScript = `(() => {
  ...
  const connect = () => {
    ...
    if (message.type === "prepare" && typeof message.sessionId === "string") {
      ...
      for (const track of stream ? stream.getAudioTracks() : []) track.enabled = !message.mic.effectiveMuted;
    ...
    socket.addEventListener("close", () => {
      stopMedia();
      show("Voice service disconnected. Run agentvoice phone again to open a new call.");
  ...
  hold.addEventListener("pointerdown", (event) => {
    if (!sessionId || hold.disabled) return;
    holding = true;
  ...
  hold.addEventListener("lostpointercapture", releaseHold);
