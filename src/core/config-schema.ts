...
export const orchestratorValuesSchema = z
  .strictObject({
    workspace: z
      .string()
      .describe(
        "The conversation workspace: native session selection and the Codex thread use this root. Tilde-expanded; relative paths use the launch directory. Default: current directory under the XDG state agentvoice/default/workspaces base, selected when the server creates its retained workspace session. --workspace overrides this value.",
      )
      .optional(),
    model: z
      .string()
...
export const voiceValuesSchema = z
  .strictObject({
    ...
    name: z
      .string()
      .describe(
        "Voice timbre, validated by Codex when voice starts. File-based launches load once per runtime generation. Database-backed workspace roles additionally support persistent MCP/API voice_set with voice-only reconnection; runtime or server restart loads the full saved revision. Use MCP/API voice_get to discover names compatible with the owned Codex runtime; managed API edits validate membership before saving. Native v1/v3 share a voice family; legacy v2 is incompatible with WebRTC. With AgentVoice's v3 default, unset honors Codex's configured realtime voice, then its v3 timbre default. Raw version:null restores the native WebRTC v1 fallback and ignores that configured voice (Codex 0.153.3); explicit voice.name still passes through.",
      )
      .optional(),
    version: z
    ...
    "delegation-ack-filler": z
      .boolean()
      .describe(
        "Let the voice agent emit a filler acknowledgement while the orchestrator works. Omitted defers to Codex's native service default. Set true or false to select it explicitly. Raw voice.extra.delegationAckFiller still overrides this value, including null for native resolution.",
      )
      .optional(),
...
const serverShape = {
  ...
  debug: z
    .boolean()
    .describe(
      "Write private runtime protocol/media logs. Loaded once per runtime generation. Default: false. --debug wins over false here.",
    )
    .optional(),
  "codex-config": z
    .array(
