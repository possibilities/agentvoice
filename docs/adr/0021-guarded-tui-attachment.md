...
`6fa32a47047d4edfb890d03552ebdf2825c53e92` on `worktree/lucky-field-8dd4`.
Its live trial did not establish reliable display of later orchestrator messages.
This restoration therefore rechecks successive owner turns and streaming text
explicitly, while leaving simultaneous voice validation outstanding.
...
The displayed history is the unaltered orchestrator conversation, including raw
realtime delegation messages. Voice-only exchanges need not create orchestrator
turns. Typed input uses native start/steer; native assistant output is mirrored to
the active realtime session. User text itself is not mirrored. No prompt overlay
or separate transcript is introduced to change this behavior.
...
verified warm resume, successive owner turns and in-progress text display, idle typed turns, active turn steering,
