...
AgentVoice supplies `agentvoice threads --json`: version 4 metadata-only output
...
optional native Unix-second turn start/terminal timestamps. Missing timestamps
remain absent rather than falling back to observation time. Each row also carries
canonical collaboration task identity observed
...
both, or has missing/conflicting evidence. Earlier versions remain historical
inputs for compatible consumers but cannot provide this timing contract; versions
1 and 2 also cannot provide canonical task identity. AgentVoice owns no HUD store,
...
worker identity and rollout ordering, and [ADR 0100](adr/0100-export-authoritative-turn-timing.md)
for the timestamp boundary.
